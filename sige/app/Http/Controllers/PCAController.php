...
class PCAController extends Controller
{
    ...
    public function cadasaluno(){
        ...
        $query = DB::table('funcionarios')
           ...
           foreach ($query as $funcionario) {
            $imagem_fun = $funcionario->imagem_fun;
        return view('cadasaluno', ['imagem_fun' => $imagem_fun , 'user' => $user, 'alunos' => $alunos, 'search2' => $search2,'rota' => $rota, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos]);
           }
    }else{
        return redirect('acessdenied');
        }
    ...
    public function cadafuncionario(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
            return view('cadafuncionario', ['imagem_fun' => $imagem_fun,'user' => $user, 'rota' => $rota, 'turmas' => $turmas, 'disciplinas' => $disciplinas, 'cursos' => $cursos, 'classes' => $classes]);
                                ...
                                foreach ($query3 as $classes) {
                                    foreach ($query4 as $turmas) {
                                        foreach ($query5 as $disciplinas) {
                                            ...
                                            $funcionarios->cursos()->attach($cursos->id, ['classe_id' => $classes->id, 'turma_id' => $turmas->id]);                                                                             
    ...
    public function permissoes(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('permissoes', ['imagem_fun' => $imagem_fun,'search' => $search, 'funcionarios' => $funcionarios, 'allfunc' => $allfunc, 'user' => $user, 'rota' => $rota ]);
    }  
}
...
public function update_permissao(Request $request){
  ...
  foreach ($query as $q) {
    ...
    public function turmas(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('cadaturma', ['imagem_fun' => $imagem_fun,'user' => $user, 'rota' => $rota, 'cursos' => $cursos, 'classes' => $classes]);
    }
}
    ...
    public function cursos(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('cursos', ['imagem_fun' => $imagem_fun,'user' => $user, 'rota' => $rota ]);
       }
    }
    ...
    public function definicao(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('definicoes', ['imagem_fun' => $imagem_fun, 'user' => $user, 'rota' => $rota ]);
    }
}
    ...
    public function classes(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('classes', ['imagem_fun' => $imagem_fun,'user' => $user, 'rota' => $rota ]);
    }
}
    ...
    public function gerenciaralunos(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
            return view('gerenciaralunos', ['imagem_fun','user' => $user, 'rota' => $rota, 'alunos' => $alunos, 'search' => $search, 'turmas' => $turmas, 'cursos' => $cursos,'classes' => $classes, 'query1' => $query1 , 'alunos2' => $alunos2
        ]);
           }
        }
    ...
    public function gerenciarturmas(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('gerenturmas', ['imagem_fun' => $imagem_fun,'user' => $user, 'rota' => $rota,'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'query' => $query]);
    }
}
    ...
    public function perfil2(Request $request){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
            ...
            return view('ver_perfil2', ['imagem_fun' => $imagem_fun,'user' => $user, 'funcionarios' => $funcionarios]);   
    ...
    public function defi_admin(){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
            return view('admin_defi', ['imagem_fun' => $imagem_fun, 'user' => $user, 'funcionario' => $funcionario]);   
        }
...
//gerenciar funcionarios
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('g_func', ['imagem_fun' => $imagem_fun, 'user' => $user, 'rota' => $rota,
        ...
        public function destroy_funcionarios($id){
            ...
            $classe_dados = DB::table('classe_curso_funcionario_turma')
            ->where('classe_curso_funcionario_turma.funcionario_id', $id)
            ->get();   
    ...
    public function edit_alunos($id){
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
                return view('edit.edit_aluno', ['imagem_fun' => $imagem_fun, 'nome_turma' => $nome_turma,'nome_classe' => $nome_classe, 'nome_curso' => $nome_curso,  'user' => $user, 'aluno' => $aluno, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'rota' => $rota]);
            }
        }
        }
       }
    ...
    $turmas = Turma::all();
      ...
      foreach ($query as $funcionarios) {
       $imagem_fun = $funcionarios->imagem_fun;
                return view('adicionar.adicionar_turma', ['imagem_fun' => $imagem_fun,  'user' => $user, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'rota' => $rota, 'funcionario' => $funcionario]);
    ...
    foreach ($query as $turma) {
        foreach ($query2 as $curso) {
            foreach ($query3 as $classe) {
                ...
                $funcionario->cursos()->attach($curso->id, ['classe_id' => $classe->id, 'turma_id' => $turma->id]);                                                                             
...
public function edit_func($id){
    $user = auth()->user();
       ...
       $query = DB::table('funcionarios')
       ->select('classe_curso_funcionario_turma.id','classes.nome_classe', 'cursos.nome_curso', 'turmas.nome_turma')
       ->join('classe_curso_funcionario_turma', 'funcionarios.id', 'classe_curso_funcionario_turma.funcionario_id')
       ->join('classes', 'classes.id', 'classe_curso_funcionario_turma.classe_id')
       ->join('turmas', 'turmas.id', 'classe_curso_funcionario_turma.turma_id')
       ->join('cursos', 'cursos.id', 'classe_curso_funcionario_turma.curso_id')
       ->where('funcionarios.id', $id)
       ->get();
       ...
       foreach ($query10 as $funcionarios) {
        $imagem_fun = $funcionarios->imagem_fun;
                ...
                return view('edit.edit_func', ['imagem_fun' => $imagem_fun, 'user' => $user, 'query' => $query, 'nome_disciplina' => $nome_disciplina,  'funcionario' => $funcionario, 'rota' => $rota, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'disciplinas' => $disciplinas]);
   ...
   foreach ($query10 as $funcionarios) {
    $imagem_fun = $funcionarios->imagem_fun;
            ...
            return view('edit.edit_func', ['imagem_fun' => $imagem_fun, 'user' => $user, 'funcionario' => $funcionario, 'rota' => $rota, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'disciplinas' => $disciplinas]);
...
public function delete_turm($id){
     DB::table('classe_curso_funcionario_turma')
        ->where('id', $id)->delete();
...
public function edit_turm_func($id){
    ...
    $turmas = Turma::all();
       ...
       $dados = DB::table('classe_curso_funcionario_turma')
       ->select('classe_curso_funcionario_turma.id', 'classes.nome_classe', 'turmas.nome_turma', 'cursos.nome_curso')
       ->join('turmas', 'turmas.id', 'classe_curso_funcionario_turma.turma_id')
       ->join('cursos', 'cursos.id', 'classe_curso_funcionario_turma.curso_id')
       ->join('classes', 'classes.id', 'classe_curso_funcionario_turma.classe_id')
       ->where('classe_curso_funcionario_turma.id', $id)
       ->get();
       ...
       foreach ($query as $funcionario) {
        $imagem_fun = $funcionario->imagem_fun;
        return view('edit.edit_turma_func',  ['imagem_fun' => $imagem_fun, 'user' => $user, 'turmas' => $turmas, 'classes' => $classes, 'cursos' => $cursos, 'rota' => $rota, 'funcionario' => $funcionario, 'nome_turma' => $nome_turma
...
public function update_turm_func(Request $request){
    ...
    foreach ($query as $turma) {
        DB::table('classe_curso_funcionario_turma')->where('id', $request->id)->update(['turma_id' => $turma->id]);
        foreach ($query2 as $curso) {
            DB::table('classe_curso_funcionario_turma')->where('id', $request->id)->update(['curso_id' => $curso->id]);
            foreach ($query3 as $classe) {
                DB::table('classe_curso_funcionario_turma')->where('id', $request->id)->update(['classe_id' => $classe->id]);
